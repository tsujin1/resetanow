...
import { Search, MoreHorizontal, FilePlus, FileBadge, Users, Calendar } from "lucide-react";
...
export default function Patients() {
  ...
  const getDaysSinceVisit = (lastVisit: string) => {
    const days = Math.floor((new Date().getTime() - new Date(lastVisit).getTime()) / (1000 * 60 * 60 * 24));
    return days;
  };
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-600">Manage patient records and medical history</p>
            </div>
          </div>
        </div>
      ...
      <div className="grid gap-6 sm:grid-cols-3">
        ...
        <Card className="border-slate-200 shadow-sm">
          <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 bg-slate-50/50 py-4">
            <CardTitle className="text-sm font-medium text-slate-700">Male Patients</CardTitle>
          ...
          <CardContent className="pt-6">
            ...
            <p className="text-xs text-slate-500 mt-1">
                {Math.round((dummyPatients.filter(p => p.gender === 'Male').length / dummyPatients.length) * 100)}% of total
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="border-slate-200 shadow-sm">
          <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 bg-slate-50/50 py-4">
            <CardTitle className="text-sm font-medium text-slate-700">Female Patients</CardTitle>
            <Users className="h-4 w-4 text-slate-500" />
          </CardHeader>
          <CardContent className="pt-6">
            <div className="text-2xl font-bold text-slate-900">
            ...
            <p className="text-xs text-slate-500 mt-1">
                {Math.round((dummyPatients.filter(p => p.gender === 'Female').length / dummyPatients.length) * 100)}% of total
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="border-slate-200 shadow-sm">
          ...
          <CardContent className="pt-6">
            <div className="text-2xl font-bold text-slate-900">
                {Math.round(dummyPatients.reduce((sum, p) => sum + p.age, 0) / dummyPatients.length)}
            </div>
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between gap-3 bg-white p-1 rounded-lg">
            ...
            <div className="text-sm text-slate-500 hidden sm:block">
                Showing <span className="font-semibold text-slate-900">{filteredPatients.length}</span> results
            </div>
        </div>
        ...
        <Card className="border-slate-200 bg-white shadow-sm">
            ...
            <Table>
                ...
                <TableRow className="border-b border-slate-200 bg-slate-50/50 hover:bg-slate-50/50">
                    <TableHead className="h-12 font-semibold text-slate-700 pl-6">Patient Name</TableHead>
                    <TableHead className="h-12 font-semibold text-slate-700">Age</TableHead>
                    <TableHead className="h-12 font-semibold text-slate-700">Gender</TableHead>
                    <TableHead className="h-12 font-semibold text-slate-700">Contact</TableHead>
                    <TableHead className="h-12 font-semibold text-slate-700">Last Visit</TableHead>
                    <TableHead className="h-12 text-right font-semibold text-slate-700 pr-6">Actions</TableHead>
                </TableRow>
                </TableHeader>
                <TableBody>
                    ...
                    return (
                        ...
                        <TableCell className="py-4 pl-6">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-slate-500 font-mono">ID: {patient._id}</p>
                            </div>
                            </div>
                        </TableCell>
