...
export default function Dashboard() {
  ...
  const handleExportCSV = () => {
    ...
    csvRows.push(`Total Patients,${summaryMetrics.patients.total},+${summaryMetrics.patients.growth}% New`);
    csvRows.push(`Prescriptions,${summaryMetrics.prescriptions.total},${summaryMetrics.prescriptions.monthly} Issued`);
    csvRows.push(`Medical Certificates,${summaryMetrics.medCerts.total},${summaryMetrics.medCerts.monthly} Issued`);
    csvRows.push(`Total Revenue,${summaryMetrics.revenue.total},${summaryMetrics.revenue.monthly} Earned`);
    ...
    weeklyData.forEach(day => {
      ...
      csvRows.push(`${day.name},${day.prescription},${day.medCert},${totalDaily}`);
    });
    csvRows.push("");
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-600">Overview of your clinic's performance</p>
            </div>
          </div>
        </div>
        {/* ACTION BUTTONS */}
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card className="border-slate-200 shadow-sm">
          ...
          <CardContent className="pt-6">
            <div className="text-2xl font-bold text-slate-900">{summaryMetrics.patients.total.toLocaleString()}</div>
            <p className="text-xs text-slate-500 flex items-center mt-1">
              <ArrowUpRight className="h-3 w-3 text-emerald-600 mr-1" /> 
              <span className="text-emerald-600 font-medium">+{summaryMetrics.patients.growth}%</span> 
        ...
        <Card className="border-slate-200 shadow-sm">
          <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 bg-slate-50/50 py-4">
            <CardTitle className="text-sm font-medium text-slate-700">Prescriptions</CardTitle>
            <FileText className="h-4 w-4 text-slate-500" />
          </CardHeader>
          <CardContent className="pt-6">
            <div className="flex justify-between items-end">
                <div>
                    <div className="text-2xl font-bold text-slate-900">{summaryMetrics.prescriptions.monthly}</div>
        ...
        <Card className="border-slate-200 shadow-sm">
          <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 bg-slate-50/50 py-4">
            <CardTitle className="text-sm font-medium text-slate-700">Med Certs</CardTitle>
            <FileBadge className="h-4 w-4 text-slate-500" />
          </CardHeader>
          <CardContent className="pt-6">
            <div className="flex justify-between items-end">
                <div>
                    <div className="text-2xl font-bold text-slate-900">{summaryMetrics.medCerts.monthly}</div>
                    <p className="text-xs text-slate-500 mt-1">Issued this month</p>
                </div>
                <div className="bg-slate-100 text-slate-600 px-2 py-1 rounded text-[10px] font-semibold border border-slate-200">
        ...
        <Card className="border-slate-200 bg-slate-900 text-slate-50 shadow-md">
          ...
          <CardContent className="pt-6">
              <div className="flex justify-between items-end">
                <div>
                    <div className="text-2xl font-bold text-white">₱{summaryMetrics.revenue.monthly.toLocaleString()}</div>
                ...
                <div className="bg-slate-800 text-slate-300 px-2 py-1 rounded text-[10px] font-semibold border border-slate-700">
                    All Time: ₱{(summaryMetrics.revenue.total / 1000000).toFixed(1)}M
                </div>
             </div>
          </CardContent>
        </Card>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-7">
        ...
        <Card className="col-span-4 border-slate-200 shadow-sm">
          <CardHeader className="border-b border-slate-100 bg-slate-50/50">
            <CardTitle className="text-base font-semibold text-slate-900">Weekly Analysis</CardTitle>
            <CardDescription className="text-sm">Revenue breakdown by service type</CardDescription>
          </CardHeader>
          <CardContent className="pt-6 pl-0">
            ...
            <div className="flex justify-center gap-6 text-xs text-slate-600 mt-4">
                <div className="flex items-center gap-2">
                    <div className="w-3 h-3 rounded bg-slate-900"></div> Prescriptions
                </div>
                <div className="flex items-center gap-2">
        ...
        <Card className="col-span-4 lg:col-span-3 border-slate-200 shadow-sm">
          <CardHeader className="border-b border-slate-100 bg-slate-50/50">
            <CardTitle className="text-base font-semibold text-slate-900">Revenue Sources</CardTitle>
          ...
          <CardContent className="pt-6">
              <div className="h-[300px] w-full relative">
                ...
                <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center">
                    ...
                    <p className="text-2xl font-bold text-slate-900">₱{(summaryMetrics.revenue.monthly / 1000).toFixed(0)}k</p>
                </div>
              </div>
              <div className="flex justify-center gap-6 text-sm text-slate-700 -mt-4">
                <div className="flex items-center gap-2">
                    <div className="w-3 h-3 rounded-full bg-slate-900"></div> Rx (71%)
                </div>
                <div className="flex items-center gap-2">
                    <div className="w-3 h-3 rounded-full bg-slate-500"></div> Certs (29%)
                </div>
              </div>
          </CardContent>
        </Card>
      ...
      <Card className="border-slate-200 shadow-sm">
        <CardHeader className="border-b border-slate-100 bg-slate-50/50">
            <div className="flex items-center justify-between">
                <div>
                    <CardTitle className="text-base font-semibold text-slate-900">Monthly Growth</CardTitle>
                    <CardDescription className="text-sm">Revenue trend over the last 7 months</CardDescription>
                </div>
                <div className="text-right hidden sm:block">
                    <p className="text-2xl font-bold text-slate-900">₱{currentMonth.revenue.toLocaleString()}</p>
                    <p className={`text-xs font-medium flex items-center justify-end ${monthGrowthStyles}`}>
                        {monthDiff > 0 ? <ArrowUpRight className="h-3 w-3 mr-1" /> : <TrendingUp className="h-3 w-3 mr-1 rotate-180" />}
                        {monthDiff > 0 ? "+" : ""}₱{Math.abs(monthDiff).toLocaleString()} vs last month
                    </p>
                </div>
            </div>
        </CardHeader>
